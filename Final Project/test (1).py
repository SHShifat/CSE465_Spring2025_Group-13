...
Original file is located at
    https://colab.research.google.com/drive/15xsCzYfpDEkNQJOEpl25KwxRWnz3oXuC
"""
...
daily_sales = daily_sales.asfreq('D')
daily_sales['Sales'] = daily_sales['Sales'].fillna(daily_sales['Sales'].median())
daily_sales['Sales'] = np.log1p(daily_sales['Sales'])  # Same transformation
daily_sales.reset_index(inplace=True)
...
X_lstm = create_sequences(scaled_data, window_size, horizon)
X_lstm_input = X_lstm[:, :, 0].reshape((X_lstm.shape[0], X_lstm.shape[1], 1))  # Only sales

X_xgb = X_lstm.reshape((X_lstm.shape[0], X_lstm.shape[1] * X_lstm.shape[2]))

model_lstm = load_model('models/lstm_model.keras')  # or .h5 if you used HDF5
model_xgb = XGBRegressor()
model_xgb.load_model('models/xgb_model.json')

lstm_preds = model_lstm.predict(X_lstm_input).flatten()
xgb_preds = model_xgb.predict(X_xgb)
hybrid_preds = (lstm_preds + xgb_preds) / 2
...
true_sales = inverse_sales(X_lstm[:, -1, 0])
predicted_sales = inverse_sales(hybrid_preds)

print("\nSample Predictions:")
for i in range(10):
    print(f"True: {true_sales[i]:.2f} | Predicted: {predicted_sales[i]:.2f}")